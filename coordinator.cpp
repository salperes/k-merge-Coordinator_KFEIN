...
sMQTTinitial mqttinitial;
sMQTTPeriodical mqttperiodical;
sMQTTinterrim mqttinterrim;
...
bool SendToWPS(uint8_t LORAWPSindex); // WPS �zerine veri yollama fonksiyonu
...
void PrepareDataToWPS(uint8_t LORAWPSindex, uint8_t flagtype, int16_t Rssi); // WPS'e yollanacak verileri haz�rlama fonksiyonu
...
int main(int argc, const char* argv[])
{
	...
	else
	{
		...
		while (!force_exit)
		{
			...
			// LostCheck i�lemi i�in timer ayar� 300 sn
			if ((millis() - LostCheckTimer) > 300000) // 300 sn'de bir Lost_Check() yap�lacak
			{
				LostCheck();
				LostCheckTimer = millis();
			}
			if (LostCheckTimer > millis()) LostCheckTimer = millis();
...
bool SendMessageMQTT(uint8_t index, uint8_t MessageType)
{
...

	{
		...
		MQTTMsg[15] = (uint8_t)LORAcomm[index].coordinatordata.rssi;						// 15 - COORDINATOR RSSI ---- D�ZELT�P YOLLA.
...
bool RWRegisteredWPSCfg(char ReadWrite, uint8_t WPSAddress)
{
	...
	if (ReadWrite == 'A' || ReadWrite == 'a')
	{
		...
		//newver	{
		//newver		if (WPSLostCheckStruct[i].WPSAddress == WPSAddress)  // WPSAddress ile WPSLostCheckStruct.WPSAddress e�itli�ine bak
		//newver		if (WPSLostCheckStruct[i].OnRegisteredWPSFile) return true; // && !WPSLostCheckStruct[i].StatusChange) return true; // dosyada var ve status de�i�imi yoksa ��k..
		if (LORAcomm[WPSAddress].loststruct.OnRegisteredWPSFile) return true;

		//newver	if (!WPSLostCheckStruct[i].OnRegisteredWPSFile) // dosya i�erisinde yoksa dosyaya ekle...
		//newver		{
		fstream wpsfile;
		char buffer[10];
...
bool LORARead()
{
	...
	if (driver.recv((uint8_t*)&wpsdata, &len))
	{
		...
		//						
		//						   FIN veya CFG_SET DE��L �SE
		//				BU DURUMDA PERIODICAL-VEHICLE_ST-INTERRIM PAKED�DIR
		...
		if ((wpsdata.flagtype != FLAG_FIN) && (wpsdata.flagtype != FLAG_CONFIGURATION_SET))
		{
			...
			///////////////////////////////////////////////////////////////
			// CHECKSUM OK �SE ACK_OK PAKETD� YOLLA....
			///////////////////////////////////////////////////////////////
			...
			if ((LORAWPSindex == 57) && (sequence < 5236))
			...
			// ACK_OK VEYA CHG_RQ PAKED� YOLLA SONUCUNU ACTSend i�erisine yaz...
		...
		//						
		//						   FIN veya CFG_SET �SE
		//						  HABERLE�ME SONA ER�YOR
		//						  MQTT VER�S� YOLLANIYOR
		////////////////////////////////////////////////////////////////////////////////////
		else if ((wpsdata.flagtype == FLAG_CONFIGURATION_SET || wpsdata.flagtype == FLAG_FIN) && LORAcomm[from].FirstPacketReceived)
		{
			...
			//FIN PAKED�NDEN �NCE ACK_OK SEND ED�LMEM�� �SE RETRANSM�T PAKED� YOLLA -- UNCOMPLETED FIN
...
void PrepareDataToWPS(uint8_t LORAWPSindex, uint8_t flagtype, int16_t Rssi)
{
	...
	LORAcomm[LORAWPSindex].coordinatordata.flagtype = flagtype; // PAKET T�P�
	...
	// WPS �ZER�NDEN ALINAN S�NYAL Rssi
	LORAcomm[LORAWPSindex].coordinatordata.rssi = (uint8_t)(-1 * Rssi);

	//hangi parametreler de�i�ecek?

...
vector <cWPSLostCheckObj> WPSLostCheckObj;
...
vector <cWPS_Config> wpsnewconfig;
...
char client_id[10] = { 'c','o','o','r','d','0','0','0','0','0' }; //MQTT Client ID ilk değeri
...
char* int_to_str(int i, char b[], int format_);		// datetime_f için kullanılan fonksiyn
...
void WPS_DATA_print(uint8_t data_from_wps_temp[WPS_PACKET_LENGHT], uint8_t len, uint8_t from, uint8_t to, uint8_t id, uint8_t flags, int16_t Rssi, uint16_t checksum, uint16_t checksumreceived); // debug print
void PrepareDataToWPS(uint8_t LORAWPSindex, uint8_t flagtype); // WPS'e yollanacak verileri hazırlama fonksiyonu
...
{
	cout << endl << " Battery Voltage = " << (float)readvoltage()*23.48*3/32768 << " V" << endl;
	...
	int_to_str(coordinator.serial, stringnumber, 0);
	for (int i = 0; i< strlen(stringnumber); i++)
	{
		client_id[9 - i] = stringnumber[strlen(stringnumber) - 1 - i];
	}
	cout << client_id << endl;
	cout << coordinator.ServerAddress << ":" << coordinator.ServerPort << endl;
...
bool SendMessageMQTT(uint8_t LORAWPSindexOnFIN, uint8_t MessageType)
{
	...
	case MSG_INITIAL:
	{
		...
		*/
		...
		MQTTMsg[27] = (uint8_t)(coordinator.serial);				// 27 - SERIAL
	...
	case MSG_VEHICLE_STATUS:
	{
		...
		MQTTMsg[15] = (uint8_t)(LORAcomm[LORAWPSindexOnFIN].data_to_wps[4]*(-1));		// 15 - COORDINATOR RSSI ---- DÜZELTİP YOLLA.
		...
		MQTTMsg[23] = 0;// (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a7];	// 23 - WPS VECTOR X MSB
		MQTTMsg[24] = 0;//(uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a8];	// 24 - WPS VECTOR X LSB
		MQTTMsg[25] = 0;//(uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a9];	// 25 - WPS VECTOR Y MSB
		MQTTMsg[26] = 0;//(uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a10];	// 26 - WPS VECTOR Y LSB
		MQTTMsg[27] = 0;//(uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a11];	// 27 - WPS VECTOR Z MSB
		MQTTMsg[28] = 0;//(uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a12];	// 28 - WPS VECTOR Z LSB
		...
		//		MQTTMsg[34] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[12];	// 34 - WPS RSSI MSB
		//		MQTTMsg[35] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[13];	// 35 - WPS RSSI LSB
		
		
		MQTTMsg[36] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[14];	// 36 - WPS Serial 3rd Octet
		MQTTMsg[37] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[15];	// 37 - WPS Serial 2nd Octet
		MQTTMsg[38] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[16];	// 38 - WPS Serial 1st Octet
		MQTTMsg[39] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[17];	// 39 - WPS HW Version
		MQTTMsg[40] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[18];	// 40 - WPS SW Version Major
		MQTTMsg[41] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[19];	// 41 - WPS SW Version Minor
		MQTTMsg[42] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[20];	// 42 - WPS Vector Variance Limit
		MQTTMsg[43] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[21];	// 43 - WPS Vehicle Detection Count
		MQTTMsg[44] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[22];	// 44 - WPS Radio TX Power
		MQTTMsg[45] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[23];	// 45 - WPS Sensor Gain
		MQTTMsg[46] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[24];	// 46 - WPS Sleep Count
		MQTTMsg[47] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[25];	// 47 - WPS Frequency 4th Octet
		MQTTMsg[48] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[26];	// 48 - WPS Frequency 3rd Octet
		MQTTMsg[49] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[27];	// 49 - WPS Frequency 2nd Octet
		MQTTMsg[50] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[28];	// 50 - WPS Frequency 1st Octet
		MQTTMsg[51] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[29];	// 51 - WPS ERROR CODE
		MQTTMsg[52] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[30];	// 52 - BLE TIMEOUT
		MQTTMsg[53] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[31];	// 53 - LoRA RETRY COUNT
	...
	{
		MQTTMsg[9] = (uint8_t)coordinator.address;								// 09 - COORDINATOR_ADDRESS
		MQTTMsg[10] = 0;														// 10 - COORDINATOR_BATTERY_VALUE MSB
		MQTTMsg[11] = 0;														// 11 - COORDINATOR_BATTERY_VALUE LSB
		MQTTMsg[12] = (uint8_t)coordinator.SWversion_Major;						// 12 - COORDINATOR SW_VERSION MAJOR
		MQTTMsg[13] = (uint8_t)coordinator.SWversion_Minor;						// 13 - COORDINATOR SW_VERSION MINOR
		MQTTMsg[14] = (uint8_t)coordinator.HWversion;							// 14 - COORDINATOR HW_VERSION
		MQTTMsg[15] = (uint8_t)(LORAcomm[LORAWPSindexOnFIN].data_to_wps[4] * (-1));		// 15 - COORDINATOR RSSI ---- DÜZELTİP YOLLA.
		...
		MQTTMsg[21] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[5];	// 21 - WPS SENSOR TEMP MSB
		MQTTMsg[22] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[6];	// 22 - WPS SENSOR TEMP LSB
		MQTTMsg[23] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a7];	// 23 - WPS VECTOR X MSB
		MQTTMsg[24] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a8];	// 24 - WPS VECTOR X LSB
		MQTTMsg[25] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a9];	// 25 - WPS VECTOR Y MSB
		MQTTMsg[26] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a10];	// 26 - WPS VECTOR Y LSB
		MQTTMsg[27] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a11];	// 27 - WPS VECTOR Z MSB
		MQTTMsg[28] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[a12];	// 28 - WPS VECTOR Z LSB
		MQTTMsg[29] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[7];	// 29 - WPS VECTOR VARIANCE
		MQTTMsg[30] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[8];	// 30 - WPS Vehicle Status
		MQTTMsg[31] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[9];	// 31 - WPS BATT VALUE MSB
		MQTTMsg[32] = (uint8_t)LORAcomm[LORAWPSindexOnFIN].data_from_wps[10];	// 32 - WPS BATT VALUE LSB
		...
		while (!fin.eof())
		{
			...
			for (int i = 0; i < n; i++) // n = #of tokens
			{
				...
				if (string(token[i]) == "#")
				{
					...
					WPSLostCheckObj[coordinator.registered_wps].WPSAddress = (uint8_t)(atoi(token[1]));
					WPSLostCheckObj[coordinator.registered_wps].WPSSerial = (uint32_t)(atoi(token[2]));
					WPSLostCheckObj[coordinator.registered_wps].SleepCount = (uint8_t)(atoi(token[3]));
					WPSLostCheckObj[coordinator.registered_wps].IsLost = (uint8_t)(atoi(token[4]));
					WPSLostCheckObj[coordinator.registered_wps].WasLost = (uint8_t)(atoi(token[5]));
					...
					printf("%d.	WPS Address =  %d	- WPS Serial = %.6X	- WPS SleepCount = %d	- WPS Lost = ",
						(coordinator.registered_wps + 1),
						WPSLostCheckObj[coordinator.registered_wps].WPSAddress,
						WPSLostCheckObj[coordinator.registered_wps].WPSSerial,
			...
			if (WPSLostCheckObj[i].WPSAddress == WPSAddress)  // WPSAddress ile WPSLostCheckObj.WPSAddress eşitliğine bak
				...
				else
				{
					if (!WPSLostCheckObj[i].OnRegisteredWPSFile) // dosya içerisinde yoksa dosyaya ekle...
					{
						...
						wpsfile << "# ";
						int_to_str(WPSLostCheckObj[i].WPSAddress, buffer, 0);
						wpsfile << buffer << " ";

						int_to_str(WPSLostCheckObj[i].WPSSerial, buffer, 0);
						wpsfile << buffer << " ";

						int_to_str(WPSLostCheckObj[i].SleepCount, buffer, 0);
						wpsfile << buffer << " ";

						int_to_str(WPSLostCheckObj[i].IsLost, buffer, 0);
						wpsfile << buffer << " ";

						int_to_str(WPSLostCheckObj[i].WasLost, buffer, 0);
						wpsfile << buffer << " " << endl;
...
bool ReadCoordinatorConfig()
{
	...
	while (!fin.eof())
	{
		...
		for (int i = 0; i < n; i++) // n = #of tokens
		{
			...
			{
				coordinator.serial = (uint16_t)(atoi(token[i + 2]));
...
bool PrepareCoordinator()
{
	...
	else
	{
		...
		char stringnumber[10];
		int_to_str(sixteenbit, stringnumber, 0);
		...
		int_to_str(coordinator.address, stringnumber, 0);
		...
		int_to_str(coordinator.registered_wps, stringnumber, 0);
...
bool LORARead(void)
{
	...
	if (driver.recv(data_from_wps_temp, &len))
	{
		...
		{
			checksumreceived = ((uint16_t)data_from_wps_temp[len - 2]) * 256 + data_from_wps_temp[len - 1];
			checksum = 0;

			LORAWPSindex = LORAPacketIndex(from, NOT_FIN_PACKET);
			for (uint8_t i = 0; i < WPS_PACKET_LENGHT - 2; i++)
			{
			...
			if (checksum != checksumreceived)
			{
				...
				//for (int xxx = 0; xxx < WPS_PACKET_LENGHT; xxx++) (printf("%d	:	%x\n", xxx, data_from_wps_temp[xxx]));
		...
		if ((data_from_wps_temp[1] == FLAG_FIN))// || (data_from_wps_temp[1] == FLAG_CONFIGURATION_SET))
		{
			...
			for (uint8_t i = 0; i < (coordinator.max_wps); i++)
			{
				...
				if (WPSLostCheckObj[i].WPSAddress == 0)
				{
					...
					WPSLostCheckObj[i].WasLost = 0;
					WPSLostCheckObj[i].LostTimer = millis();
					WPSLostCheckObj[i].SleepCount = LORAcomm[LORAWPSindexOnFIN].data_from_wps[24];
					WPSLostCheckObj[i].WPSSerial = ((uint32_t)(LORAcomm[LORAWPSindexOnFIN].data_from_wps[14]) << 16) | ((uint32_t)(LORAcomm[LORAWPSindexOnFIN].data_from_wps[15]) << 8) | LORAcomm[LORAWPSindexOnFIN].data_from_wps[16];
					WPSLostCheckObj[i].SignalSend = true;
					WPSSignalSending++;
	...
	if (checksum == checksumreceived)
	{
		...
		printf("-------------------------------------------\n");
		//PrepareDataToWPS(from,FLAG_ACK_OK);
	...
	WPS_DATA_print(LORAcomm[LORAWPSindex].data_from_wps, len, from, to, id, flags, Rssi, checksum, checksumreceived);
...
void PrepareDataToWPS(uint8_t LORAWPSindex, uint8_t flagtype)
{
	...
	LORAcomm[LORAWPSindex].data_to_wps[2] = WPSADDRESS; // HANGİ WPS ADRESİNE 
														//LORAcomm[LORAWPSindex].data_to_wps[2] = WPS_BROADCAST_ADDRESS; // TÜM WPS ADRESLERİNE 
	...
	LORAcomm[LORAWPSindex].data_to_wps[3] = 0; //VECTOR0/ WPS RESETLENSİN Mİ? (1=VECTOR0, 2=WPS SW)
	LORAcomm[LORAWPSindex].data_to_wps[4] = (int8_t)driver.lastRssi(); // WPS ÜZERİNDEN ALINAN SİNYAL Rssi

	

	LORAcomm[LORAWPSindex].data_to_wps[5] = 32;			//YENİ CONFİG VAR MI
	//0=NONE
	//BIT 0 = RADIO TX POWER
	//BIT 1 = VEHICLE DETECTION COUNT
	//BIT 2 = VECTOR VARIANCE LIMIT
	//BIT 3 = SLEEP COUNT
	//BIT 4 = GAIN
	//BIT 5 = COORDINATOR ADDRESS & RADIO FREQUENCY
	//BIT 6 = WPS ADDRESS
	//BIT 7 = NETWORK ID
	...
	LORAcomm[LORAWPSindex].data_to_wps[8] = WPSADDRESS; // YENİ WPS ADRESİ
	LORAcomm[LORAWPSindex].data_to_wps[9] = def_COORDINATOR_ADDRESS; // YENİ COORDINATOR ADRESİ (Default=0x00)
	LORAcomm[LORAWPSindex].data_to_wps[10] = 30; // YENİ WPS_SLEEP_COUNT 
	LORAcomm[LORAWPSindex].data_to_wps[11] = 10; //YENİ VEKTÖR SAPMA ORANI  (Default=10)

	// BURAYA DİKKAT.... BLE TIME OUT 5-8 BİTLERİNE YAZILACAK...
	LORAcomm[LORAWPSindex].data_to_wps[12] = 3 << 4; //YENİ BLE TIME OUT 5-8 Bitlerine Yaz
	LORAcomm[LORAWPSindex].data_to_wps[12] = LORAcomm[LORAWPSindex].data_to_wps[12] | 1; //YENİ ARAÇ TESPİT KATSAYISI (Default=5)  1-4 Bitlerine yaz
	
	LORAcomm[LORAWPSindex].data_to_wps[13] = 13; //WPS RADIO_TX POWER (MIN=5, MAX=23, DEFAULT=13)

	float f = 433.92;
	uint8_t *frq_pointer = (uint8_t*)&f;
	for (uint8_t i = 0; i < sizeof(f); i++)
	{
		//printf("%d\n",frq_pointer[i]);
		LORAcomm[LORAWPSindex].data_to_wps[14 + i] = frq_pointer[i];
	}
...
void WPS_DATA_print(uint8_t data_from_wps_temp[WPS_PACKET_LENGHT], uint8_t len, uint8_t from, uint8_t to, uint8_t id, uint8_t flags, int16_t Rssi, uint16_t checksum, uint16_t checksumreceived)
{
	...
	{
		frq_pointer[i] = data_from_wps_temp[25 + i];
	}
	printf(" WPS Frequency       : %3.2f MHz\n", wps_frq);
	printf(" WPS Address         : %d\n", from);
	printf(" WPS Serial          : %.2X:%.2X:%.2X\n", data_from_wps_temp[14], data_from_wps_temp[15], data_from_wps_temp[16]);
	printf(" WPS Software Version: %d.%d\n", data_from_wps_temp[18], data_from_wps_temp[19]);
	printf(" WPS Sequence Number : %d\n", (uint32_t)((data_from_wps_temp[2] * 256 * 256 + data_from_wps_temp[3] * 256 + data_from_wps_temp[4])));
	printf(" WPS Vector Variance : %d % \n", data_from_wps_temp[7]);
	float temperature;
	int16_t tmp = (int16_t)((data_from_wps_temp[5] << 8) | data_from_wps_temp[6]);

	if (data_from_wps_temp[17] == 1 | data_from_wps_temp[17] == 2)
	{
	...
	printf(" Battery Level       : %d % \n", data_from_wps_temp[11]);
	printf(" Battery Voltage     : %3.2f V \n", ((float)((uint16_t)(data_from_wps_temp[9]) << 8 | data_from_wps_temp[10])*3.3 * 2 / 1024));
	printf(" Vehicle Detected    : "); if (data_from_wps_temp[8]) { printf("True\n"); }
	else { printf("False\n"); }
	//printf(" Config Applied      : "); if (data_from_wps_temp[22]) { printf("True\n"); }
	//else { printf("False\n"); }
	printf(" RADIO_RSSI WPS SIDE : %d dB\n", (int16_t)(data_from_wps_temp[12] << 8 | data_from_wps_temp[13]));
	printf(" RADIO_RSSI_COR SIDE : %d dB\n", Rssi);
	printf(" WPS HW Version      : %d (", data_from_wps_temp[17]);
	switch (data_from_wps_temp[17])
	{
...
char* datetime_f(char dates[])
{
	...
	int_to_str(_YEAR, sYear, 1);
	int_to_str(_MONTH, sMon, 1);
	int_to_str(_DAY, sDay, 1);
	int_to_str(_HOUR, sHour, 1);
	int_to_str(_MINUTE, sMin, 1);
	int_to_str(_SECOND, sSec, 1);
...
char* int_to_str(int i, char b[], int format_)
{
